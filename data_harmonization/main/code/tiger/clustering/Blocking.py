...
def Blocking(is_train=True, if_word_2vec=False, if_min_lsh=True):
    ...
    def createTokens(profile: dict, features_to_deduplicate=None):
        ...
        output = " ".join(map(lambda x: x.strip(), filter(
            lambda e: e and not e.isspace(), output)))
        return Row(id=profile['id'], shingles=output)

    # cleanse the dataframe <id, features [shingles]>
    cleansed_df = df.rdd.map(lambda r: createTokens(
        r.asDict())).toDF(['id', 'shingles'])
    cleansed_df.show()
    # Space Tokenizer
    ...
    regexTokensDF = regexTokenizer.transform(
        tokensDF).select("id", "reg_tokens")
    regexTokensDF.show()
    # remove stop words
    remover = StopWordsRemover(inputCol="reg_tokens", outputCol="clean_tokens")
    cleansedTokensDF = remover.transform(
        regexTokensDF).select("id", "clean_tokens")
    ...
    if if_word_2vec:
        if is_train:
            ...
            resultsDF = model.transform(
                cleansedTokensDF).select("id", "vector")
        else:
        ...
        if is_train:
            cv = CountVectorizer(
                inputCol="clean_tokens", outputCol="vector", vocabSize=200 * 10000, minDF=1.0)
            cv_model = cv.fit(cleansedTokensDF)
            cv_model.write().overwrite().save(os.path.abspath(__file__) +
                                              "/../../../../../../data_harmonization/main/model/model.countvec")
            resultsDF = cv_model.transform(
                cleansedTokensDF).select("id", "vector")
        else:
            cv_model = CountVectorizerModel.load(os.path.abspath(
                __file__)+"/../../../../../../data_harmonization/main/model/model.countvec")
            resultsDF = cv_model.transform(
                cleansedTokensDF).select("id", "vector")

    if if_min_lsh:
        # 1.MinHashLSH
        brp = MinHashLSH(inputCol='vector',
                         outputCol='hashes', numHashTables=4.0)
    ...
    similarDF = model.approxSimilarityJoin(resultsDF, resultsDF, threshold, distCol="JaccardDistance").filter("JaccardDistance != 0")\
        .select(col("datasetA.id").alias("id"),
